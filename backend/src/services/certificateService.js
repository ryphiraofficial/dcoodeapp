...
export const generateBulkCertificates = async (batchId, staffId, description) => {
  ...
  // Persist the description on the batch so students can retrieve it later
  await Batch.findByIdAndUpdate(batchId, { certificateDescription: description.trim() });
...
// Student: get their batch name + certificate description (used before cert is generated)
export const getBatchCertificateInfo = async (studentId) => {
  const student = await Student.findById(studentId).populate('batch');
  if (!student) throw { statusCode: 404, message: 'Student not found.' };
  if (!student.batch) throw { statusCode: 404, message: 'No batch assigned to this student.' };

  const batch = student.batch;
  return {
    batchId: batch._id,
    batchName: batch.name,
    certificateDescription: batch.certificateDescription || '',
  };
};
