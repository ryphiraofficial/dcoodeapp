...
export const generate = async (req, res, next) => {
  try {
    ...
    if (!description || !description.trim()) {
      return sendError(res, 400, 'Description is required to generate a certificate.');
    }
    const certificate = await certificateService.generateCertificate(req.params.studentId, req.userId, description.trim());
...
export const generateBulk = async (req, res, next) => {
  try {
    const { description } = req.body;
    if (!description || !description.trim()) {
      return sendError(res, 400, 'Description is required to generate certificates.');
    }
    const result = await certificateService.generateBulkCertificates(req.params.batchId, req.userId, description.trim());
...
// Student: get their batch name + description (to know what description will appear on their cert)
export const getBatchInfo = async (req, res, next) => {
  try {
    const info = await certificateService.getBatchCertificateInfo(req.userId);
    return sendSuccess(res, 200, 'Batch certificate info fetched successfully', { info });
  } catch (err) {
    if (err.statusCode) return sendError(res, err.statusCode, err.message);
    next(err);
  }
